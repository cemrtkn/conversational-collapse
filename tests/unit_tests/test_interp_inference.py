...
from api.interp_inference import InterpInference, DEFAULT_MODEL
...
class TestInterpInference:
    ...
    def test_initialization_with_defaults(self, mock_language_model):
        """Test that InterpInference initializes correctly with default parameters."""
    ...
    def test_initialization_with_custom_device(self, mock_language_model):
        ...
        interp = InterpInference(model=DEFAULT_MODEL, device="cuda:1")
    ...
    def test_initialization_with_cpu_device(self, mock_language_model):
        ...
        interp = InterpInference(model=DEFAULT_MODEL, device="cpu")
    ...
    def test_generate_calls_model_correctly(self, mock_language_model):
        """Test that generate calls the underlying model with correct parameters."""
        # Setup mock
        ...
        mock_model_instance.generate.return_value.__enter__ = MagicMock(return_value=mock_tracer)
        mock_model_instance.generate.return_value.__exit__ = MagicMock(return_value=False)
        mock_model_instance.generator.output.save.return_value = torch.tensor([[1, 2, 3, 4, 5, 6, 7, 8]])
        mock_model_instance.tokenizer.decode.return_value = "Generated text"

        interp = InterpInference(model=DEFAULT_MODEL)
        result = interp.generate("Test prompt", max_new_tokens=100, input_len=3)
    ...
    def test_generate_from_messages_with_chat_template(
        ...
        mock_model_instance.tokenizer.apply_chat_template.return_value = "formatted prompt"
        mock_model_instance.tokenizer.return_value = {"input_ids": torch.tensor([[1, 2, 3, 4, 5]])}
        ...
        interp = InterpInference(model=DEFAULT_MODEL)
        response = interp.generate_from_messages(sample_messages, max_new_tokens=50)
    ...
    def test_generate_from_messages_fallback_without_chat_template(
        ...
        """Test generate_from_messages falls back to concatenation without chat template."""
        # Setup mock without apply_chat_template
        mock_model_instance = MagicMock()
        mock_language_model.return_value = mock_model_instance
        mock_model_instance.tokenizer = MagicMock(spec=["decode", "__call__"])
        mock_model_instance.tokenizer.return_value = {"input_ids": torch.tensor([[1, 2, 3]])}

        # Mock generate context manager
        mock_model_instance.generate.return_value.__enter__ = MagicMock()
        mock_model_instance.generate.return_value.__exit__ = MagicMock(return_value=False)
        mock_model_instance.generator.output.save.return_value = torch.tensor([[1, 2, 3, 4, 5]])
    ...
    def test_generate_with_custom_parameters(self, mock_language_model):
        ...
        mock_model_instance.generate.return_value.__enter__ = MagicMock()
        mock_model_instance.generate.return_value.__exit__ = MagicMock(return_value=False)
        mock_model_instance.generator.output.save.return_value = torch.tensor([[1, 2, 3, 4]])
